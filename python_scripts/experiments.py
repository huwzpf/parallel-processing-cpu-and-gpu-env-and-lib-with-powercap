...
def write_powercap_conf(run_parameters: RunParameters, config_path: str = "powercap.conf"):
    """Write power capping configuration file consumed by cudampilib.
    Uses CONTINOUS_EQUAL strategy with cpu_min_powercap, gpu_min_powercap and optional cpu_time_window_us.
    """
    # Only write config when at least one relevant field is provided
    if (
        run_parameters.cpu_min_powercap is None
        and run_parameters.gpu_min_powercap is None
        and run_parameters.cpu_time_window_us is None
    ):
        return
